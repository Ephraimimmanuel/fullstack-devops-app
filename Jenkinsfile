pipeline {
    ...
    steps {
        bat '''
        ssh -o StrictHostKeyChecking=no -i "C:/Users/ephra/Downloads/ephraim-frontend-app.pem" ubuntu@ec2-3-110-193-157.ap-south-1.compute.amazonaws.com "docker stop backend || true && docker rm backend || true && docker load < /home/ubuntu/backend.tar && docker run -d -p 5000:5000 --name backend devops-backend"
        '''
    }
}
